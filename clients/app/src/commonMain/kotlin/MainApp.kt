...
fun MainApp() {
    MaterialTheme {
        Surface(
            ...
            // Handle PKCE callback on app load (web)
            LaunchedEffect(Unit) {
                ...
                if (callback != null) {
                    ...
                    if (pkce != null && pkce.state == state) {
                        ...
                        val res = api.exchangeAuthorizationCode(code, pkce.codeVerifier, AppConfig.webRedirectUri())
                        val token = res.token
            ...
            when (currentScreen) {
                is AppScreen.Home -> WelcomeScreen(
                    ...
                    onOpenLogin = {
                        // Fallback to local LoginScreen (Password Grant) if PKCE cannot be started
...
private fun WelcomeScreen(
    ...
    Column(
                    ...
                    onClick = {
                        ...
                        scope.launch {
                            try {
                                ...
                                val url = OAuthPkceService.buildAuthorizeUrl(pkce, AppConfig.webRedirectUri())
                                uriHandler.openUri(url)
                            } catch (t: Throwable) {
                                // Fallback: open local Login screen (Password Grant)
                                onOpenLogin()
                            }
                        }
                    },
        ...
        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(12.dp)) {
            OutlinedButton(
                onClick = { uriHandler.openUri(AppConfig.registerUrl()) },
                modifier = Modifier.weight(1f)
            ) { Text("Registrieren (Keycloak)") }

            OutlinedButton(
                onClick = { uriHandler.openUri(AppConfig.loginUrl()) },
                modifier = Modifier.weight(1f)
        ...
        OutlinedButton(
            onClick = { uriHandler.openUri(AppConfig.desktopDownloadUrl()) },
